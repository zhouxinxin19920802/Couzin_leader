...
a1_d_test = integrate.quad(
    lambda x: (A1 + (k1 - A1) / (1 + np.exp(B1_case1 * (x - x1_short))))
    if (A1 + (k1 - A1) / (1 + np.exp(B1_case1 * (x - x1_short)))) > task_value
    else 0,
    0,
    50,
)[0]
print("a1_d_test", a1_d_test)
a1_r_test = integrate.quad(
    lambda x: A2 + (k2_case1 - A2) / (1 + np.exp(B2_case1 * (x - x2_short)))
    if A2 + (k2_case1 - A2) / (1 + np.exp(B2_case1 * (x - x2_short))) > task_value
    else 0,
    50,
    100,
)[0]
...
a1_d = integrate.quad(lambda x: (A1 + (k1 - A1) / (1 + np.exp(B1_case1 * (x - x1_short)))), t_ss1_reverse, 50)[0]
print("a1_d", a1_d)
a1_r = integrate.quad(lambda x: A2 + (k2_case1 - A2) / (1 + np.exp(B2_case1 * (x - x2_short))), 50, t_ss1)[0]
print("a1_r", a1_r)
sigma1 = (a1_d + a1_r) / (integrate.quad(lambda x: 50, t_ss1_reverse, t_ss1)[0])

resilience1_improved = sigma1 * rho1 * (delta1_improved + zeta1) * tm.time_amend(t_ss1 - t_ss1_reverse, absolute_time)
...
a2_d = integrate.quad(lambda x: A1 + (k1 - A1) / (1 + np.exp(B1_case2 * (x - x1_short))), t_ss2_reverse, 50)[0]
a2_r = integrate.quad(lambda x: A2 + (k2_case2 - A2) / (1 + np.exp(B2_case2 * (x - x2_short))), t_ss2, 100)[0]
sigma2 = (a2_d + a2_r) / (integrate.quad(lambda x: 50, t_ss2_reverse, t_ss2)[0])


resilience2_improved = sigma2 * rho2 * (delta1_improved + zeta2) * tm.time_amend(t_ss2 - t_ss2_reverse, absolute_time)
...
# 毁伤前稳态时间
t_ss3_reverse, min_var3_reverse = tm.calculate(t_case1_reverse, y_t3_reverse)
a3_d = integrate.quad(lambda x: A1 + (k1 - A1) / (1 + np.exp(B1_case1 * (x - x1_short))), t_ss3_reverse, 50)[0]
a3_r = integrate.quad(lambda x: A2 + (k2_case3 - A2) / (1 + np.exp(B2_case1 * (x - x2_short))), 50, t_ss3)[0]
sigma3 = (a3_d + a3_r) / (integrate.quad(lambda x: 50, t_ss3_reverse, t_ss3)[0])

resilience3_improved = sigma3 * rho3 * (delta1_improved + zeta3) * tm.time_amend(t_ss3 - t_ss3_reverse, absolute_time)
...
# 毁伤前稳态时间
t_ss4_reverse, min_var4_reverse = tm.calculate(t_case1_reverse, y_t4_reverse)
a4_d = integrate.quad(lambda x: A1 + (k1 - A1) / (1 + np.exp(B1_case1 * (x - x1_short))), t_ss4_reverse, 50)[0]
a4_r = integrate.quad(lambda x: A2 + (k2_case4 - A2) / (1 + np.exp(B2_case1 * (x - x2_short))), 50, t_ss4)[0]
sigma4 = (a4_d + a4_r) / (integrate.quad(lambda x: 50, t_ss4_reverse, t_ss4)[0])

resilience4_improved = sigma4 * rho4 * (delta1_improved + zeta4) * tm.time_amend(t_ss4 - t_ss4_reverse, absolute_time)
...
a5_d = integrate.quad(lambda x: A1 + (k1 - A1) / (1 + np.exp(B1_case1 * (x - x1_short))), t_ss5_reverse, 50)[0]
a5_r = integrate.quad(lambda x: A2 + (k2_case5 - A2) / (1 + np.exp(B2_case1 * (x - x2_short))), 50, t_ss5)[0]
sigma5 = (a5_d + a5_r) / (integrate.quad(lambda x: 50, t_ss5_reverse, t_ss5)[0])

resilience5_improved = sigma5 * rho5 * (delta1_improved + zeta5) * tm.time_amend(t_ss5 - t_ss5_reverse, absolute_time)
...
a6_d = integrate.quad(lambda x: A1 + (k1 - A1) / (1 + np.exp(B1_case1 * (x - x1_short))), t_ss6_reverse, 50)[0]
a6_r = integrate.quad(lambda x: A2 + (k2_case6 - A2) / (1 + np.exp(B2_case1 * (x - x2_short))), 50, t_ss6)[0]
sigma6 = (a6_d + a6_r) / (integrate.quad(lambda x: 50, t_ss6_reverse, t_ss6)[0])
resilience6_improved = sigma6 * rho6 * (delta1_improved + zeta6) * tm.time_amend(t_ss6 - t_ss6_reverse, absolute_time)
...
print(
    "resilience_re:{}\t,amend:{}\t".format(
        sigma6 * rho6 * (delta1_improved + zeta6 + 1 - pow(tau6, rho6 - delta6)),
        tm.time_amend(t_ss6 - t_ss6_reverse, absolute_time),
    )
)
...
# 案例 7
a7_d = integrate.quad(
    lambda x: (A1 + (k1 - A1) / (1 + np.exp(B1_case1 * (x - x1_long))))
    if (A1 + (k1 - A1) / (1 + np.exp(B1_case1 * (x - x1_long)))) > task_value
    else 0,
    0,
    100,
)[0]
a7_r = integrate.quad(
    lambda x: (A2 + (k2_case7 - A2) / (1 + np.exp(B2_case1 * (x - x2_long))))
    if (A2 + (k2_case7 - A2) / (1 + np.exp(B2_case1 * (x - x2_long)))) > task_value
    else 0,
    100,
    200,
)[0]
...
sigma7 = (a7_d + a7_r) / (integrate.quad(lambda x: 50, t_ss7_reverse, t_ss7)[0])
resilience7_improved = sigma7 * rho7 * (delta1_improved + zeta7) * tm.time_amend(t_ss7 - t_ss7_reverse, absolute_time)
...
a8_d = integrate.quad(lambda x: A1 + (k1 - A1) / (1 + np.exp(B1_case1 * (x - x1_long))), t_ss8_reverse, 100)[0]
a8_r = integrate.quad(lambda x: A2 + (k2_case8 - A2) / (1 + np.exp(B2_case1 * (x - x2_long))), 100, t_ss8)[0]
sigma8 = (a8_d + a8_r) / (integrate.quad(lambda x: 50, t_ss8_reverse, t_ss8)[0])
resilience8_improved = sigma8 * rho8 * (delta1_improved + zeta8) * tm.time_amend(t_ss8 - t_ss8_reverse, absolute_time)
...
a9_d = integrate.quad(lambda x: A1 + (k1 - A1) / (1 + np.exp(B1_case9 * (x - x1_long))), t_ss9_reverse, 100)[0]
a9_r = integrate.quad(lambda x: A2 + (k2_case9 - A2) / (1 + np.exp(B2_case9 * (x - x2_long))), 100, t_ss9)[0]
sigma9 = (a9_d + a9_r) / (integrate.quad(lambda x: 50, t_ss9_reverse, t_ss9)[0])
resilience9_improved = sigma9 * rho9 * (delta1_improved + zeta9) * tm.time_amend(t_ss9 - t_ss9_reverse, absolute_time)
...
a10_d = integrate.quad(lambda x: A1 + (k1 - A1) / (1 + np.exp(B1_case1 * (x - x1_long))), t_ss10_reverse, 100)[0]
a10_r = integrate.quad(lambda x: A2 + (k2_case10 - A2) / (1 + np.exp(B2_case1 * (x - x2_long))), 100, t_ss10)[0]
sigma10 = (a10_d + a10_r) / (integrate.quad(lambda x: 50, t_ss10_reverse, t_ss10)[0])

resilience10_improved = (
    sigma10 * rho10 * (delta1_improved + zeta10) * tm.time_amend(t_ss10 - t_ss10_reverse, absolute_time)
)
...
a11_d = integrate.quad(lambda x: A1 + (k1 - A1) / (1 + np.exp(B1_case1 * (x - x1_long))), t_ss11_reverse, 100)[0]
a11_r = integrate.quad(lambda x: A2 + (k2_case11 - A2) / (1 + np.exp(B2_case1 * (x - x2_long))), 100, t_ss11)[0]
sigma11 = (a11_d + a11_r) / (integrate.quad(lambda x: 50, t_ss11_reverse, t_ss11)[0])

resilience11_improved = (
    sigma11 * rho11 * (delta1_improved + zeta11) * tm.time_amend(t_ss11 - t_ss11_reverse, absolute_time)
)
...
a12_d = integrate.quad(lambda x: A1 + (k1 - A1) / (1 + np.exp(B1_case1 * (x - x1_long))), t_ss12_reverse, 100)[0]
a12_r = integrate.quad(lambda x: A2 + (k2_case12 - A2) / (1 + np.exp(B2_case1 * (x - x2_long))), 100, t_ss12)[0]
sigma12 = (a12_d + a12_r) / (integrate.quad(lambda x: 50, t_ss12_reverse, t_ss12)[0])
resilience12_improved = (
    sigma12 * rho12 * (delta1_improved + zeta12) * tm.time_amend(t_ss12 - t_ss12_reverse, absolute_time)
)

print("t_ss12:{}\t,t_ss12_reverse:{}\t,sigma12:{}\t".format(t_ss12, t_ss12_reverse, sigma12))
